...
const AnalyticsTile: React.FC<AnalyticsTileProps> = ({ num, tag, expandedText, color1, color2 }) => {
  ...
  return (
    <div ref={tileRef} onClick={handleToggleExpand} className={`transition-transform duration-500 ease-in-out ${containerClass}`}>
      <div 
        style={{ backgroundImage: `linear-gradient(to right, ${color1}, ${color2})` }} className={`p-4 py-6 flex text-white flex-col items-center animate-fadeIn justify-center shadow-lg rounded-xl bg-gradient-to-r from-${color1} to-${color2} cursor-pointer`}
